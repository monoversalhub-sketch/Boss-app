...
export default function PayButton({ order, customer, tailor, publicKey }) {
  ...
  // MISSING-05: Payment confirmation polling
  ...
  // MISSING-05: Poll for payment confirmation every 15s for up to 3 minutes
  // Starts when customer copies account details (indicating intent to pay)
  useEffect(() => {
    if (!isPolling) return;
    const interval = setInterval(async () => {
      try {
        ...
        const newBal = data.order.price - data.order.deposit - data.order.paid;
        if (newBal < currentBal - 1) { // balance decreased = payment received
    ...
    const timeout = setTimeout(() => {
      clearInterval(interval);
      setIsPolling(false);
      setPollMsg("Payment not yet confirmed. If you transferred, please check back in a few minutes.");
    }, 180000);
    return () => { clearInterval(interval); clearTimeout(timeout); };
  }, [isPolling, currentBal, order.id]);
  ...
  async function copyDetails() {
    ...
    // MISSING-05: Start polling for payment confirmation after copy
    setIsPolling(true);
    setCurrentBal(balance);
    setPollMsg("");
    try {
          ...
          {hasVA ? (
            <div style={{
              ...
              <div style={{ padding: "16px 20px" }}>
                {isPolling && (
                  <div style={{ textAlign: "center", color: C.sub, fontSize: 12, marginBottom: 10, fontWeight: 500 }}>
                    🔄 Checking for your payment every 15 seconds…
                  </div>
                )}
                {pollMsg && !isPolling && (
                  <div style={{ textAlign: "center", color: C.sub, fontSize: 12, marginBottom: 10, lineHeight: 1.5 }}>
                    {pollMsg}
                  </div>
                )}
