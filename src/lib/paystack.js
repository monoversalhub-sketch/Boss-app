...
export function openPaystackPopup({ email, amount, name, phone, ref, onSuccess, onClose }) {
  ...
  if (!publicKey) {
    // alert() removed — blocks UI thread and is not production-appropriate.
    // Log clearly for the developer; call onClose so the UI resets gracefully.
    console.error("[paystack] NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY is not set. Add it to .env.local to enable card payments.");
    onClose?.();
    return;
  }
  ...
  const interval = setInterval(() => {
    ...
    if (attempts > 30) {
      clearInterval(interval);
      // alert() removed — call onClose so the flow resets; log for debugging.
      console.error("[paystack] SDK failed to load after 3s. Check internet connection.");
      onClose?.();
    }
  }, 100);
