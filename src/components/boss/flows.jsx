...
export function AddOrderFlow({open,onClose,prefilledCid}){
  ...
  async function save(){
    ...
    try{
      ...
      setCustomers(next);
      const result=await db.setCustomers(next);
      if(!result?.ok) toast("⚠️ Saved locally — sync failed. Check connection.");
...
export function OrderDetailFlow({open,onClose,orderId,tailor}){
  ...
  return(
    <>
    <Flow open={open} onClose={onClose} title={customer.name} action="Delete" onAction={()=>setConfirmDelete(true)}>
...
export function AddClientFlow({open,onClose,onDone}){
  ...
  async function save(){
    ...
    try{
      ...
      setCustomers(next);
      const result=await db.setCustomers(next);
      if(!result?.ok) toast("⚠️ Client saved locally — sync failed.");
